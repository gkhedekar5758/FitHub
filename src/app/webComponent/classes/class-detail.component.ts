...
export class ClassDetailComponent implements OnInit {
  ...
  constructor(private route:ActivatedRoute,private coachService:CoachService,private classService:ClassService) { }
  ...
  ngOnInit(): void {
    ...
    this.getClassList().then(()=>{
      this.route.params.forEach((param:Params) => {
       ...
       this.classDetail=this.classes.find( c=> c.className === param['name']);
  ...
  populateCoachInformation = (classID:number) =>{
    ...
    this.coachService.getCoachesByClassID(classID)
    .subscribe(response=>{
      //console.log(response);
